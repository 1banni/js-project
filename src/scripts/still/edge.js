...
export default class Edge {
  constructor(x1, y1, x2, y2, layers, color = 'white', lineWidth=10) {
    this.x1 = x1;
    this.y1 = y1;
    this.x2 = x2;
    this.y2 = y2;
  ...
  intersects (particle) {
    ...
    if (this.vertical) {
      if (Math.abs(particle.x - this.x1 - gap) < radi) { // is distance greater < radius
      ...
      if (Math.abs(particle.y - this.y1 - gap) < radi) { // is distance greater < radius
